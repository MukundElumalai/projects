...
class Ui_Dialog_enrolled_course_disp(object):
    ...
    def load_data(self):
        ...
        cur=conn.cursor()
        cur.execute("select disp_enrolled_courses(%s);",[self.user])
        c = conn.cursor('cursor1')
        x=c.fetchall()
        self.tableWidget.setRowCount(0)
        self.tableWidget.insertRow(0)
        header=['COURSE_ID','COURSE_NAME','STATUS','FINAL_GRADE']
    ...
    def retranslateUi(self, Dialog):
        ...
        Dialog.setWindowTitle(_translate("Dialog", self.name+"-ENROLLED COURSES"))
