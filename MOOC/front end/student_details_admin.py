...
class Ui_Dialog_student_disp(object):
    ...
    def load_data(self):
        ...
        cur=conn.cursor()
        cur.execute("select disp_students_admin();")
        c = conn.cursor('cursor1')
        x=c.fetchall()
        self.tableWidget.setRowCount(0)
        self.tableWidget.insertRow(0)
        header=['USER_NAME','FIRST_NAME','LAST_NAME','COURSE_ID','COURSE_NAME']
    ...
    def retranslateUi(self, Dialog):
        ...
        Dialog.setWindowTitle(_translate("Dialog", self.name+"-STUDENTS"))
