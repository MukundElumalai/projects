...
class Ui_Dialog_course_disp(object):
    ...
    def load_data(self):
        ...
        cur=conn.cursor()
        cur.execute("select disp_courses();")
        c = conn.cursor('cursor1')
        x=c.fetchall()
        self.tableWidget.setRowCount(0)
        self.tableWidget.insertRow(0)
        header=['COURSE ID','NAME','DESCRIPTION','COMMITMENT','COURSE_PRICE','MIN_GRADE','LECTURER']
    ...
    def retranslateUi(self, Dialog):
        ...
        Dialog.setWindowTitle(_translate("Dialog", self.name+"-COURSES"))
